...
def vis_labelled_tracks(args):
    ...
    if os.path.isfile(args.TrackLabellingExportPth):
        ...
        if os.path.exists(args.HomographyNPY) and os.path.exists(
...
def get_proper_tracks(args, gp, verbose=True):
    ...
    tracks_meter = group_tracks_by_id(
        pd.read_pickle(tracks_meter_path), gp=gp, verbose=verbose
    )
    tracks["index_mask"] = tracks_meter["trajectory"].apply(
        lambda x: track_resample(x, return_mask=True, threshold=args.ResampleTH)
    )
    ...
    if gp:
        ...
        for p in args.MetaData["roi"]:
            x, y = p[0], p[1]
            new_point = Homography.reproj_point(
                args,
                x,
                y,
                method,
    ...
    tracks["ROI_mask"] = tracks.apply(
        lambda row: get_in_roi_points(
            row.trajectory[row.index_mask], poly_path, return_mask=True
        ),
        axis=1,
    )
    ...
    for i, row in tqdm(tracks.iterrows(), total=len(tracks), disable=not verbose):
        ...
        if len(np.unique(int_indexes)) > 1 and is_monotonic(
            get_in_roi_points(traj, poly_path)
        ):  # more than one interactions with ROI
            mask.append(True)
            counter += 1
...
def cluster_prop_tracks_based_on_str(tracks, args):
    ...
    for mi in moi_clusters.keys():
        ...
        clt = sklearn.cluster.KMeans(
            n_clusters=min(num_cluster, len(starts)), n_init=100
        )
...
def make_uniform_clt_per_moi(tracks, args):
    ...
    for mi in np.unique(tracks.moi):
        tracks_mi = tracks[tracks["moi"] == mi].sort_values(
            by="traj_len", ascending=False
        )
        ...
        print(
            f"nums to take: {num_tracks_to_get_from_clt_per_moi} moi:{mi} num_clt:{len(uni_vals)} tot_tracks_chosen:{len(uni_vals)*num_tracks_to_get_from_clt_per_moi}"
        )
        for uv in uni_vals:
            idxs_mi_clt = tracks_mi[tracks_mi.clt == uv].index[
                :num_tracks_to_get_from_clt_per_moi
            ]
...
def extract_common_tracks_multi(args_top, arg_mcs):
    ...
    trackss = [
        get_proper_tracks(args, gp=gp, verbose=False) for args in tqdm(args_flatten)
    ]
    ...
    for mi in moi_clusters.keys():
        ...
        for c_label in cluster_labels:
            ...
            indexes_mi_cl = np.array([i for i in range(len(tracks_mi_cl))]).reshape(
                -1, 1
            )
    ...
    tracks_labelleds = [
        group_tracks_by_id(pd.read_pickle(tracks_path), gp=gp, verbose=False).loc[
            tracks_temp.index
        ]
        for tracks_path, tracks_temp in zip(tracks_paths, trackss)
    ]
...
class MyPoly:
    def __init__(self, roi, roi_group):
        ...
        for i in range(len(roi) - 1):
            self.lines.append(
                sympy.Segment(sympy.Point(roi[i]), sympy.Point(roi[i + 1]))
            )
    ...
    def distance_angle_filt(self, p_main, p_second):
        ...
        for i, line in enumerate(self.lines):
            ...
            if d_secn < d_main:
                ...
                if (
                    np.abs(float(imaginary_line.angle_between(line)) - np.pi / 2)
                    < min_angle
                ):
    ...
    def doIntersect(self, track, ret_points):
        ...
        for seg in self.lines:
            roi_segments.append(
                [Point(list(seg.points[0])), Point(list(seg.points[1]))]
            )
