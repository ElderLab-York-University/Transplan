...
def setup(args):
    ...
    if not env_name in get_conda_envs():
        ...
        print("here I am 1")
        os.system(f"conda run --live-stream -n {args.Detector} conda install pip==22.3.1")
        os.system(f"conda run -n {args.Detector} python3 -m pip install torch==1.11.0+cu113 torchvision==0.12.0+cu113  -f https://download.pytorch.org/whl/torch_stable.html")
        print("here I am 2")
        # os.system(f"conda run --live-stream -n {args.Detector} pip install --upgrade setuptools")
        os.system(f"conda run --live-stream -n {args.Detector} python3 -m pip install -U openmim")
        print("here I am 2.3")
        os.system(f"conda run --live-stream -n {args.Detector} mim install mmcv-full==1.5.0")
        print("here I am 2.6")
        os.system(f"conda run --live-stream -n {args.Detector} python3 -m pip install timm==0.6.11 mmdet==2.28.1")
        print("here I am 3")
        os.system(f"conda run --live-stream -n {args.Detector} python3 -m pip install opencv-python termcolor yacs pyyaml scipy tqdm")
        print("here I am 4")
