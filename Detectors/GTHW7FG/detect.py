...
def get_fine_grain_index_form_label(label):
  clean_label = remove_text_in_parentheses(label)
  clean_label.split(" - ")[-1]
  name_to_idx = {"":0, "":1}
  idx = name_to_idx[clean_label]
  return idx
      ...
      for response in responses['camera_responses']:
          ...
          if (response['camera_used']==camera_num):
              ...
              for gt in response['annotations']:
                  ...
                  uuid=gt['cuboid_uuid']
                  label = gt["label"]
                  c = get_fine_grain_index_form_label(label)
