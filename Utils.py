...
def get_detection_path_from_args(args):
    ...
    return os.path.join(
        ...
        file_name
        + Puncuations.Dot
        + SubTaskMarker.Detection
        + Puncuations.Dot
        + SubTaskExt.Detection,
    )
...
def get_vis_detection_path_from_args(args):
    ...
    return os.path.join(
        ...
        file_name
        + Puncuations.Dot
        + SubTaskMarker.VisDetection
        + Puncuations.Dot
        + args.Detector
        + Puncuations.Dot
        + SubTaskExt.VisDetection,
    )
...
def get_vis_tracking_path_from_args(args):
    ...
    return os.path.join(
        ...
        file_name
        + Puncuations.Dot
        + SubTaskMarker.VisTracking
        + Puncuations.Dot
        + args.Detector
        + Puncuations.Dot
        + args.Tracker
        + Puncuations.Dot
        + SubTaskExt.VisTracking,
    )
...
def get_plot_all_traj_path(args):
    ...
    return os.path.join(
        ...
        file_name
        + Puncuations.Dot
        + SubTaskMarker.VisTrajectories
        + Puncuations.Dot
        + args.Detector
        + Puncuations.Dot
        + args.Tracker
        + Puncuations.Dot
        + SubTaskExt.VisTrajectories,
    )
...
def get_vis_labelled_tracks_path(args):
    ...
    return os.path.join(
        ...
        file_name
        + Puncuations.Dot
        + SubTaskMarker.VisLTrajectories
        + Puncuations.Dot
        + args.Detector
        + Puncuations.Dot
        + args.Tracker
        + Puncuations.Dot
        + SubTaskExt.VisLTrajectories,
    )
...
def get_metadata_path(args):
    ...
    return os.path.join(
        args.Dataset,
        file_name + Puncuations.Dot + SubTaskMarker.MetaData + Puncuations.Dot + "json",
    )
...
def get_vis_roi_path(args):
    ...
    return os.path.join(
        args.Dataset,
        "Results/Visualization",
        file_name + Puncuations.Dot + SubTaskMarker.VisROI + Puncuations.Dot + "png",
    )
...
def get_dsm_points_path(args):
    ...
    return os.path.join(
        args.Dataset,
        file_name + Puncuations.Dot + "dsm_points_aoi" + Puncuations.Dot + "xyz",
    )
...
def get_orthophoto_tif_path(args):
    ...
    return os.path.join(
        args.Dataset,
        file_name + Puncuations.Dot + "OrthoPhoto" + Puncuations.Dot + "tif",
    )
...
def get_to_ground_raster_path(args):
    ...
    return os.path.join(
        args.Dataset,
        "Results/DSM/" + Puncuations.Dot + "ToGroundRaster" + Puncuations.Dot + "pkl",
    )
...
def add_correct_roi_to_args(args):
    ...
    projected_roi = []
    for x, y in ref_roi:
        cor_img = Homography.project_point(
            args,
            x,
            y,
            args.BackprojectionMethod,
            M=M,
            GroundRaster=GroundRaster,
            TifObj=orthophoto_win_tif_obj,
        )
...
def get_projection_objs(args):
    ...
    if method == "Homography":
        ...
        if not os.path.exists(args.ToGroundRaster):
            ...
            DSM.create_raster(
                args, args.MetaData["camera"], coords, projection_dict, intrinsic_dict
            )
...
def add_localization_save_path_to_args(args):
    args.LocalizationExportPath = os.path.join(
        args.Dataset, "Results/VehicleLocalization"
    )
    return args
...
def add_cached_counter_path_to_args(args):
    if args.CachedCounterPth is None:
        ...
        if args.MultiCam:
            chached_args, chached_args_mc = get_args_mc(chached_args)
        elif args.MultiSeg:
            chached_args, chached_args_mc, chached_args_mcs = get_args_ms(chached_args)
        elif args.MultiPart:
            chached_args, chached_args_mp, chached_args_mss, chached_args_mcs = (
...
def get_CPError_path(args):
    ...
    return os.path.join(
        ...
        "Results",
        "ContactPoints",
        f"{file_name}.CPError.{args.Detector}.{args.BackprojectionMethod}.{args.TopView}.{args.ContactPoint}.csv",
    )
...
def get_CPErrorMC_path(args):
    ...
    return os.path.join(
        ...
        "Results",
        "ContactPoints",
        f"{file_name}.CPErrorMC.{args.Detector}.{args.BackprojectionMethod}.{args.TopView}.{args.ContactPoint}.csv",
    )
...
def get_sub_args(args):
    ...
    for cl in os.listdir(sp):
        if (
            (not cl.startswith("."))
            and (not cl == "Results")
            and os.path.isdir(os.path.join(sp, cl))
        ):
...
def complete_args(args):
    ...
    if (
        (not args.Detector is None)
        or args.DetPostProc
        or args.ConvertDetsToCOCO
        or args.FineTune
    ):
    ...
    if (
        args.HomographyGUI
        or args.Homography
        or args.VisHomographyGUI
        or args.VisTrajectories
        or args.VisLabelledTrajectories
        or args.Cluster
        or args.TrackPostProc
        or args.Count
        or args.VisROI
        or args.Meter
        or args.VisTrackTop
        or args.FindOptimalKDEBW
        or args.VisCPTop
        or args.EvalCount
        or args.TrackEval
        or args.DetPostProc
        or args.IntegrateCountsMC
        or args.EvalCountMSfromMC
        or args.EvalContactPoitnSelection
        or args.EvalContactPoitnSelectionMC
        or args.EvalCountCamera
        or args.ConvertDetsToCOCO
        or args.FineTune
    ):
    ...
    if (
        args.TrackLabelingGUI
        or args.VisLabelledTrajectories
        or args.Meter
        or args.ExtractCommonTracks
        or args.Count
        or args.EvalCount
    ):
    ...
    if (
        args.OffsetLocalize
        or args.TrainOffsetLocalize
        or args.DepthLocalize
        or args.TrainDepthAnythingScaler
    ):
        args = add_localization_save_path_to_args(args)
        args = add_dsm_related_path_to_args(args)
...
def check_config(args):
    ...
    try:
        ...
        pass
    try:
        os.system(f"mkdir -p {detection_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {tracking_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {homography_path}")
    except:
        pass
    try:
        ...
        pass
    try:
        os.system(f"mkdir -p {counting_path}")
    except:
        pass
    try:
        ...
        pass
    try:
        os.system(f"mkdir -p {clustering_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {segment_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {dsm_path}")
    except:
        pass
    try:
        ...
        pass
    try:
        os.system(f"mkdir -p {check_points_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {contact_point_path}")
    except:
        pass
    try:
        os.system(f"mkdir -p {pose_path}")
    except:
        pass
...
def get_numgpus_torch(env_name):
    stream = os.popen(
        f"conda run -n {env_name} --live-stream python -c 'import torch; ngpus=torch.cuda.device_count(); print(ngpus);'"
    )
    output = stream.read()
    ngpus = int(output)
    return ngpus
...
def conda_pyrun(env_name, exec_file, args):
    os.system(
        f"conda run -n {env_name} --live-stream python3 \"{exec_file}\" '{json.dumps(dict(vars(args)))}'"
    )
...
def cmm_dist(traj_a, traj_b):
    ...
    cmmlib.cmm_truncate_sides.argtypes = [
...
def minvc_dist(traj_a, traj_b):
    return np.linalg.norm(traj_a[0] - traj_b[0]) + np.linalg.norm(
        traj_a[-1] - traj_b[-1]
    )
...
def hausdorff_directed(
    traj_a, traj_b, dist_func=lambda va, vb: np.linalg.norm(va - vb)
):
