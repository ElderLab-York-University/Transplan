    ...
    for k in cam_img_dict.keys():
        ...
        for row_idx, row in cam_gt_point_df.iterrows():
            ...
            cam_img_dict[k] = cv2.drawMarker(
                cv2.putText(
                    cam_img_dict[k], str(row_idx), (int(row_eval[0]), int(row_eval[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA
                ), (int(row_eval[0]), int(row_eval[1])), color=(255, 0, 0), thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15
            )
            
        cv2.imwrite(os.path.join(KEYPOINTS_VIS_DIR, '{}_gt_point.png'.format(k)), cam_img_dict[k])
    
    for row_idx, row in gt_point_df.iterrows():
        row_eval = eval(row['ortho'])
        cv2.drawMarker(
            cv2.putText(
                orthophoto_win_np_obj, str(row_idx), (int(row_eval[0]), int(row_eval[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA
            ), (int(row_eval[0]), int(row_eval[1])), color=(0, 0, 255), thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15
        )
    cv2.imwrite(os.path.join(KEYPOINTS_VIS_DIR, 'orthophoto_gt_points.png'), cv2.cvtColor(orthophoto_win_np_obj, cv2.COLOR_BGR2RGB))
...
def create_raster(args, cam_key, coords, projection_dict, intrinsic_dict):
    ...
    for coord_idx, coord in enumerate(coords):
        local_coord = np.append(coord + np.array(projection_dict['T_localgta_gta'])[:-1, -1], [1]).reshape(4, 1)
        
        cam_coord = np.matmul(np.array(projection_dict['T_{}_localgta'.format(cam_key)]), local_coord)
        projected_coord = np.matmul(np.array(intrinsic_dict[cam_key]['intrinsic_matrix']), cam_coord)
        projected_coord = projected_coord[:-1] / projected_coord[-1]
        
        if all([0 <= np.floor(projected_coord[i,0]) <= intrinsic_dict[cam_key]['image_size'][i] - 1 for i in range(2)]) and cam_coord[2] > 0:
            img_ground_map.append((coord, cam_coord, projected_coord))
    
    pd.DataFrame(
        {
            'x_w': [i[0][0].squeeze() for i in img_ground_map], 'y_w': [i[0][1].squeeze() for i in img_ground_map], 'z_w': [i[0][2].squeeze() for i in img_ground_map], 
            'x_c': [i[1][0].squeeze() for i in img_ground_map], 'y_c': [i[1][1].squeeze() for i in img_ground_map], 'z_c': [i[1][2].squeeze() for i in img_ground_map],
            'u': [i[2][0].squeeze() for i in img_ground_map], 'v': [i[2][1].squeeze() for i in img_ground_map], 
        }
    ...
    img_ground_raster = img_ground_df.iloc[knn_idx][['x_w', 'y_w', 'z_w']].values.reshape(img_size[1], img_size[0], 3)
...
def project_to_cam(args, latitude, longitude, cam_key, k = 1):
    ...
    knn = NearestNeighbors(n_neighbors=k).fit(img_ground_raster[..., :-1].reshape(-1, 2))
...
def measure_projection_error():
    ...
    for c1, c2 in list(combinations(cam_img_dict.keys(), 2)):
        cam_gt_point_df = gt_point_df[(~gt_point_df[c1].isnull()) & (~gt_point_df[c2].isnull())][[c1, c2, 'ortho', 'vicinity']]
        
        for row_idx, row in cam_gt_point_df.iterrows():
            row_ortho = eval(row['ortho'])
            row_c1, row_c2 = eval(row[c1]), eval(row[c2])
            
            gt_ground_coord = utm.to_latlon(*rasterio.transform.xy(orthophoto_win_tif_obj.transform, row_ortho[1], row_ortho[0]), 17, northern=True)
            ...
            print(
                'point idx: {}, {}(gt: {}) -> world (gt: {}, proj: {}) -> {}(gt: {}, proj: {}), pixel error: {}, geolocation error: {}'.format(
                    row_idx, c1, row_c1, gt_ground_coord, proj_ground_coord, c2, gt_pix_coord, proj_pix_coord, round(np.sqrt(np.square(proj_pix_coord - np.array(gt_pix_coord)).sum()), 2), round(hs.haversine(gt_ground_coord, proj_ground_coord, unit=Unit.METERS), 2)
                )
            )
            cv2.drawMarker(cam_img_dict[c2], proj_pix_coord, color=[0, 0, 180], thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15)
            error_list.append((c1, c2, row_idx, round(np.sqrt(np.square(proj_pix_coord - np.array(gt_pix_coord)).sum()), 2), round(hs.haversine(gt_ground_coord, proj_ground_coord, unit=Unit.METERS), 2), row['vicinity']))
            orthophoto_proj_idx = orthophoto_win_tif_obj.index(*utm.from_latlon(*proj_ground_coord)[:2])
            cv2.drawMarker(orthophoto_win_np_obj, (orthophoto_proj_idx[1], orthophoto_proj_idx[0]), color=[180, 0, 0], thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15)
            ...
            print(
                'point idx: {}, {}(gt: {}) -> world (gt: {}, proj: {}) -> {}(gt: {}, proj: {}), pixel error: {}, geolocation error: {}'.format(
                    row_idx, c2, row_c2, gt_ground_coord, proj_ground_coord, c1, gt_pix_coord, proj_pix_coord, round(np.sqrt(np.square(proj_pix_coord - np.array(gt_pix_coord)).sum()), 2), round(hs.haversine(gt_ground_coord, proj_ground_coord, unit=Unit.METERS), 2)
                )
            )
            cv2.drawMarker(cam_img_dict[c1], proj_pix_coord, color=[0, 0, 180], thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15)
            error_list.append((c2, c1, row_idx, round(np.sqrt(np.square(proj_pix_coord - np.array(gt_pix_coord)).sum()), 2), round(hs.haversine(gt_ground_coord, proj_ground_coord, unit=Unit.METERS), 2), row['vicinity']))
            orthophoto_proj_idx = orthophoto_win_tif_obj.index(*utm.from_latlon(*proj_ground_coord)[:2])
            cv2.drawMarker(orthophoto_win_np_obj, (int(row_ortho[0]), int(row_ortho[1])), color=[180, 0, 0], thickness=2, markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA, markerSize=15)
            
    for k in cam_img_dict.keys():
        cv2.imwrite(os.path.join(KEYPOINTS_VIS_DIR, '{}_gt_proj_points.png'.format(k)), cam_img_dict[k])
        
    cv2.imwrite(os.path.join(KEYPOINTS_VIS_DIR, 'orthophoto_gt_proj_points.png'), cv2.cvtColor(orthophoto_win_np_obj, cv2.COLOR_BGR2RGB))
    
    error_df = pd.DataFrame(error_list, columns = ['src_cam', 'dst_cam', 'loc_idx', 'error[pixel]', 'error[meter]', 'vicinity'])
    error_df.to_csv(os.path.join(DATA_DIR, 'error_report.csv'))
    
    rows = []
    for g_name, g in error_df.groupby(['src_cam']):
        rows.append((g_name[0], '%.2f'%(g['error[pixel]'].mean()), '%.2f'%(g['error[pixel]'].std()), '%.2f'%(g['error[meter]'].mean()), '%.2f'%(g['error[meter]'].std()), len(g)))
    df = pd.DataFrame(rows, columns = ['src_cam', 'mean error[pixel]', 'std[pixel]', 'mean error[meter]', 'std[meter]', 'point pair count'])
    display(df)
    
    rows = []
    for g_name, g in error_df.groupby(['dst_cam']):
        rows.append((g_name[0], '%.2f'%(g['error[pixel]'].mean()), '%.2f'%(g['error[pixel]'].std()), '%.2f'%(g['error[meter]'].mean()), '%.2f'%(g['error[meter]'].std()), len(g)))
    df = pd.DataFrame(rows, columns = ['dst_cam', 'mean error[pixel]', 'std[pixel]', 'mean error[meter]', 'std[meter]', 'point pair count'])   
    display(df)
    ...
    for g_name, g in error_df.groupby(['vicinity']):
        ...
        rows.append((vicinity_str, '%.2f'%(g['error[pixel]'].mean()), '%.2f'%(g['error[pixel]'].std()), '%.2f'%(g['error[meter]'].mean()), '%.2f'%(g['error[meter]'].std()), len(g)))
    df = pd.DataFrame(rows, columns = ['vicinity', 'mean error[pixel]', 'std[pixel]', 'mean error[meter]', 'std[meter]', 'point pair count'])   
    display(df)
    ...
    for g_name, g in error_df.groupby(['loc_idx']):        
        if g['vicinity'].iloc[0] == 1:
            ...
            vicinity_str = 'beyond 100m'
        rows.append((g_name[0], '%.2f'%(g['error[pixel]'].mean()), '%.2f'%(g['error[pixel]'].std()), '%.2f'%(g['error[meter]'].mean()), '%.2f'%(g['error[meter]'].std()), vicinity_str))
    df = pd.DataFrame(rows, columns = ['loc_idx', 'error[pixel]', 'std[pixel]', 'error[meter]', 'std[meter]', 'vicinity'])    
    display(df)