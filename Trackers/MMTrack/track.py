...
def track(args, *oargs):
    ...
    # exec_path = "./Trackers/MMTrack/run.py"
...
def df_txt(df, out_path):
    ...
    with open(out_path,'w') as out_file:
        for i, row in df.iterrows():
            ...
            print('%d,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%d'%(fn, idd, score, x1, y1, x2, y2, clss),file=out_file)
...
def setup(args):
    ...
    # clone mmtracking
    src_url = "https://github.com/open-mmlab/mmtracking.git"
    rep_path = "./Trackers/MMTrack/mmtracking"
    if not "mmtracking" in os.listdir("./Trackers/MMTrack/"):
        # clone source repo
        os.system(f"git clone --recurse-submodules {src_url} {rep_path}")

    # clone mmdetection
    det_src_url = "https://github.com/open-mmlab/mmdetection.git"
    det_rep_path = "./Detectors/MMDet/mmdetection"

    if not "mmdetection" in os.listdir("./Detectors/MMDet/"):
        os.system(f"git clone {det_src_url} {det_rep_path}")
    ...
    if not env_name in get_conda_envs():
        ...
        os.system(f"conda run --live-stream -n {env_name} pip install 'mmcv-full>=1.3.17,<2.0.0' -f https://download.openmmlab.com/mmcv/dist/cu113/torch1.11.0/index.html")

        # install mmdet
        os.system(f"conda run --live-stream -n {env_name} pip install 'mmdet>=2.19.1,<3.0.0'")

        # install mmtrack 
        os.chdir(rep_path)
        os.system(f"conda run -n {env_name} --live-stream pip install -r requirements/build.txt")
