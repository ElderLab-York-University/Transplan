        ...
        for response in responses['camera_responses']:
            if (response['camera_used']==camera_num):
                for gt in response['annotations']:
                    ...
                    uuid= consistent_hash(gt['cuboid_uuid'])
                    x1=gt['left']
                    x2=x1+gt['width']
                    y1=gt['top']
                    y2=y1+gt['height']
...
def df(args):
    ...
    # data["fn"], data["id"],  data["x1"], data["y1"], data["x2"], data["y2"], data["class"] = [], [], [], [], [], [], []
    ...
    with open(file_path, "r+") as f:
        ...
        for line in lines:
            ...
            fn , id, x1, y1, x2, y2, clss, uuid = float(splits[0]), float(splits[1]), float(splits[2]), float(splits[3]),\
                                            float(splits[4]), float(splits[5]), int(splits[6]), int(splits[7])
            # fn , id, x1, y1, x2, y2, clss, uuid = float(splits[0]), float(splits[1]), float(splits[2]), float(splits[3]),\
...
def df_txt(df, out_path):
    with open(out_path,'w') as out_file:
        for i, row in df.iterrows():
            fn, idd, x1, y1, x2, y2, clss, uuid = row['fn'], row['id'], row['x1'], row['y1'], row['x2'], row['y2'], row["class"], row["uuid"]
            print('%d,%d,%.4f,%.4f,%.4f,%.4f,%d,%d'%(fn, idd, x1, y1, x2, y2, clss, uuid),file=out_file)